...
function generateDockBlocks() {
    ...
    slots.forEach((slot, index) => {
        ...
        // 8% chance to contain a bomb cell
        if (Math.random() < 0.08) {
            const solidCells = [];
...
function onPointerUp(e) {
    ...
    if (clickDuration < 220 && clickDistance < 10) {
        ...
        const rotatedMatrix = Array(cCount).fill(null).map(() => Array(rCount).fill(0));
        for (let r = 0; r < rCount; r++) {
            for (let c = 0; c < cCount; c++) {
                rotatedMatrix[c][rCount - 1 - r] = matrix[r][c];
            }
        }
        shape.matrix = rotatedMatrix;
...
function checkGameOver() {
    ...
    // Check if at least one remaining block can fit
    const anyBlockFits = activeBlocks.some(shape => canShapeFit(shape));
